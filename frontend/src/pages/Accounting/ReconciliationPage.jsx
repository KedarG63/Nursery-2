...
const HIDDEN_COLUMNS = new Set(['id', 'customer_id', 'return_item_id', 'source_id']);
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Every total below is recalculated from the original rows and compared against
            what is stored. Nothing here trusts a saved figure.
          </Typography>
        </Box>
      ...
      <Box sx={{ mt: 2, mb: 3 }}>
        {summary.balanced ? (
          <Alert severity="success" icon={<PassIcon />}>
            ...
            All {summary.checks_run} checks passed. Every cached total matches its
            underlying rows, every refund has exactly one ledger entry, and no document
            is settled for more than it is worth.
          </Alert>
        ) : (
          <Alert severity={summary.critical > 0 ? 'error' : 'warning'}>
            ...
            The exact rows are listed below.
          </Alert>
        )}
      </Box>
          ...
          <Paper sx={{ p: 2 }}>
            ...
            {summary.checks_errored > 0 && (
              <Typography variant="caption" color="error.main">
                {summary.checks_errored} check(s) failed to run
              </Typography>
            )}
          </Paper>
        </Grid>
            ...
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, width: '100%' }}>
                ...
                <Chip
                  size="small"
                  color={sev.color}
                  label={check.passed === null ? 'Could not run' : `${check.count} row${check.count === 1 ? '' : 's'}`}
                />
              </Box>
            </AccordionSummary>
            <AccordionDetails>
              {check.passed === null
                ? <Alert severity="error">This check could not run: {check.error}</Alert>
                : renderRows(check)}
            </AccordionDetails>
          </Accordion>
        );
      ...
      {clean.length > 0 && (
        <>
          <Divider sx={{ my: 3 }} />
          <Typography variant="h6" gutterBottom>Checks That Passed</Typography>
          <Paper sx={{ p: 2 }}>
            {clean.map((check) => (
